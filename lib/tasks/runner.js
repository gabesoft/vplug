...
function Runner (options) {
    ...
    this.intervalError = options.intervalError;
}
...
Runner.prototype.startRunLoop = function () {
    ...
    var duration      = moment.duration(this.interval).humanize()
      , durationError = moment.duration(this.intervalError).humanize();
    this.runTasks(function (err) {
        var interval = err ? this.intervalError : this.interval;

        if (err) {
            log.error(err.message);
            log.info(
                chalk.red('tasks failed') + '... will try again in ' +
                chalk.blue(durationError) + ' from ' +
                chalk.yellow(moment().format('hh:mm:ss')));
        } else {
            log.info(
                chalk.green('tasks complete') + '... going to sleep for ' +
                chalk.blue(duration) + ' from ' +
                chalk.yellow(moment().format('hh:mm:ss')));
        }
        setTimeout(this.startRunLoop.bind(this), interval);
    }.bind(this));
};
