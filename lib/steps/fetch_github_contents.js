'use strict';

var util    = require('util')
  , Request = require('../core/github_request').GithubRequest
  , chalk   = require('chalk');

module.exports = function (state, _, cb) {
    if (state.githubContents) { return cb(); }

    var request = new Request({
            url     : state.githubContentsUrl
          , headers : state.githubRequestHeaders
        }, state.log);

    request.get(function (err, body) {
        if (err) {
            state.log.error(err.stack || err.message);
            cb(err);
        } else {
            state.githubContents = body;
            state.log.info('fetching contents ' + chalk.green(state.githubContentsUrl));
            cb();
        }
    });

};
