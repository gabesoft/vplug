...
module.exports = function (contents) {
    ...
    data =  {
        vperc      : vperc(dirs)
      , lib        : has(dirs, 'lib') || has(dirs, 'libraries')
      , src        : has(dirs, 'src')
      , bin        : has(dirs, 'bin')
      , vimdir     : has(dirs, 'vim')
      ...
      , vimrc      : hasr(files, /^[._]?(vimrc|nvimrc|init\.vim|dot\.vimrc|rc\.vim|vimrc\.vim|startup\.vimrc|dotvimrc|home_vimrc|vim\.config|vim\.sus)$/)
      , elisp      : hasr(files, /^.+\.el$/)
      , vim        : hasr(files, /^.+\.vim$/)
      , bashrc     : hasr(files, /^[._]?(zshrc|bashrc)/)
      , install    : hasr(files, /^(install|install\.sh|installer\.bat|setup\.py|spark\.sus)$/)
